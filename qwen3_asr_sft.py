...
def parse_args():
    ...
    p.add_argument("--model_path", type=str, default="./models/Qwen3-ASR-0.6B")
    p.add_argument("--train_file", type=str, default="data/seame_train_qwen_asr.json")
...
def main():
    ...
    use_bf16 = torch.cuda.is_available() and torch.cuda.get_device_capability(0)[0] >= 0
    ...
    if 'validation' not in raw_ds:
        print("Warning: No validation file provided. Splitting training data for evaluation.")
        raw_ds = raw_ds["train"].train_test_split(test_size=0.01, seed=42)
        raw_ds = {'train': raw_ds['train'], 'validation': raw_ds['test']}
    
    ds = {split: raw_ds[split].map(make_preprocess_fn_prefix_only(processor), num_proc=4) for split in raw_ds.keys()}
